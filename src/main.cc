...
void num_format(num_t *num, char *str, int len, int mode, int mode_digits)
{
    ...
    for (;;)
    {
        ...
        if (!ep)
        { // No exponent -> expecting special number, just copy
            strcpy(str, s);
            return;
        }
        ...
        int mexp = ep - s -
                   1; // Mantissa exponent (if point placed before first digit)
        int      isexp = 1;
        int      exp = atoi(ep + 1) + mexp; // Exp to num and translate to point
                                            // before first mantissa digit
        int      elen;
        int      a, b;
        unsigned c; // Aux vars
        char    *mp;
        ...
        *(++mend) = 0;
        ...
        case FMT_MODE_NONE:
            ...
            {
                if (exp <= 0) // Number requires '0.' and zero padding after
                              // decimal point
                    b +=
                        2 - exp +
                        1; // to at least one mantissa digit after padding zeros
                if (ms)
                    b++;         // One place for sign
                isexp = b > len; // Number cannot fit without exponent
            }
            break;

        case FMT_MODE_SCI:
        case FMT_MODE_ENG: break;
        }
        ...
        mode_digits = min(mode == FMT_MODE_NONE ? b : mode_digits,
                          a - 1 - (dbp > 0 ? 0 : 1));
        ...
        { // We have digits and have room for at least one frac digit
            strcat(str, dbp > 0 ? "." : "0.");
            b = max(0, b);
            strncat(str, zeros, zfad);
            mode_digits -= zfad;
            strncat(str, mp, c = min(b, mode_digits));
            mp += c;
...
void disp_annun(int xpos, const char *txt)
{
    ...
    lcd_fill_rect(t20->x,
                  1,
                  lcd_textWidth(t20, txt),
                  lcd_lineHeight(t20) - 5,
                  0);
    lcd_puts(t20, txt);
    t20->y += 2;
...
void add_edit_key(int key)
{
    ...
    case KEY_DOT:
        if (dot || exp)
            return; // It has already dot or no dots in exponents
        len = ed_cat(dotchar, len);
        break;

    case KEY_E:
        if (exp)
            return; // It has already exponent
...
int run_fn(int key)
{
    ...
    case KEY_MUL:
        ...
        break;
    case KEY_DIV:
        num_div(&res, stack + 1, stack);
        RES2;
        break;

    case KEY_INV:
        num_div(&res, &num_one, stack);
        RES1;
        break;
    case KEY_SQRT:
        num_sqrt(&res, stack);
    ...
    case KEY_SWAP:
        ...
        break;
    case KEY_CHS:
        num_sub(&res, &num_zero, stack);
        RES1;
        break;

    case FM_DISP_FIX:
    case FM_DISP_ENG:
    case FM_DISP_SCI: fmt_mode_digits = reg_to_fmt_num(stack); stack_pop();
    case FM_DISP_NONE: fmt_mode = fnr - FM_DISP_NONE; break;
...
void handle_key(int key)
{
    ...
    if (!consumed)
    {
        ...
        case KEY_COS:
        case KEY_TAN:
        case KEY_SWAP: consumed = run_fn(key); break;

        case KEY_F1: run_help_file("/HELP/db48x.html"); break;
    ...
    if (!consumed)
    {
        ...
        case KEY_9:
        case KEY_DOT:
        case KEY_E: add_edit_key(key); break;
...
void program_init()
{
    ...
    // Pi/200
    // 0.01570796326794896619231321691639751442098584699687552910487472296153908203143104499314017412671058534
...
extern "C" void program_main()
{
    ...
    */
    ...
    {
        if ((ST(STAT_PGM_END) &&
             ST(STAT_SUSPENDED)) || // Already in off mode and suspended
            (!ST(STAT_PGM_END) &&
             key_empty())) // Go to sleep if no keys available
        {
            CLR_ST(STAT_RUNNING);
            sys_sleep();
        }
        ...
        // Clear suspended state, because now we are definitely reached the
        // active state
        ...
        {
            LCD_power_on();
            rtc_wakeup_delay(); // Ensure that RTC readings after power off will
                                // be OK
