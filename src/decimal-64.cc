...
// ----------------------------------------------------------------------------
{
    record(decimal64, "Command %+s on %p", name(cmd), obj);
    switch(cmd)
    {
    case EVAL:
...
// ----------------------------------------------------------------------------
{
    record(decimal64, "Parsing [%s]", p.source);
    ...
    p.out = rt.make<decimal64>(ID_decimal64, p.source);
...
//   This is largely inspired by the code in the DM42 SDKdemo
{
    ...
    char s[50];                 // Enough even for bid64
...
// ----------------------------------------------------------------------------
{
    ...
    char buffer[50];            // bid64 with 34 digits takes at most 42 chars
